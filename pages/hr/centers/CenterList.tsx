...
const CenterList: FC = () => {
  ...
  const handleAsm = async (asmId: number) => {
    const confirmDeletion = await Swal.fire({
      title: 'Are you sure?',
      text: 'You are about to delete this center. This action cannot be undone.',
      icon: 'warning',
    ...
    try {
      const storedToken = sessionStorage.getItem('authToken');
      const response = await axios.delete(`https://marsworld.co.in/api/hr/users/${asmId}`, {
        headers: {
          Authorization: `Bearer ${storedToken}`,
        },
      });
                  ...
                  {dataPagination(filteredItems, currentPage, perPage)?.length > 0 &&
                    dataPagination(filteredItems, currentPage, perPage).map((center) => (
                      <tr key={center.Id}>
                        <td>{center.Id}</td>
                        <td>{center.name}</td>
                        <td>{center.storeName}</td>
                        <td>{center.asmName}</td>
